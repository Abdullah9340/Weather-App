...
window.addEventListener("load",() =>{
    ...
    if(navigator.geolocation){
        navigator.geolocation.getCurrentPosition(position => {
            ...
            const api = `${proxy}http://api.weatherapi.com/v1/current.json?key=7b7a2331f73a43ce8ce195400202212&q=${lat},${long}`;
...
function onPress(e){
    if(temp.textContent.substring(temp.textContent.length- 1) === "F"){
        temp.textContent = ((parseFloat(temp.textContent) - 32) * 5/9).toFixed(2) + " C";
    }else if(temp.textContent.substring(temp.textContent.length- 1) === "C"){
        temp.textContent = ((parseFloat(temp.textContent) * 9/5) + 32).toFixed(1) + " F";
    }
}
